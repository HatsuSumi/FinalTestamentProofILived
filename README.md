...
一个炫酷的个人作品集网站，展示独立游戏开发者在编程和美术两个领域的全栈才能。
...
- 🎬 **多媒体模态框** - 动画作品视频播放 + 图片作品横向滚动画廊 + 过程图标签
- 📊 **卡片序号** - 自动编号，筛选时动态更新
- 🎨 **过程图展示** - 支持展示创作过程（草稿 → 线稿 → 上色），灵活标记任意阶段
...
- **代码总行数**：8,188 行（不含空行、注释）
  - CSS：3,395 行（41.5%）
  - TypeScript：3,279 行（40.0%）
  - JSON：1,113 行（13.6%）
  - HTML：401 行（4.9%）

- **字符总数**：245,260 字符（不含注释）
  - TypeScript：124,538 字符（50.8%）
  - CSS：73,352 字符（29.9%）
  - JSON：33,211 字符（13.5%）
  - HTML：14,159 字符（5.8%）
